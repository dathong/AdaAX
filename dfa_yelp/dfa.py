...
def verifyDfa(tFunc):
    ...
    for p in paths:
        res.append("".join(p))
    print('verifying res = ', res)
    for p in res:
        # if re.search("10(1)+01", p) == None:
        # if not p.count("1") == 5 or not p.endswith("1"):
        # if 'abcd' not in p \
        #         or not p.endswith('abcd'):
            # if '11111' not in p \
            #     or not p.endswith('11111'):
            # return False
        if not verifyGrid(p):
            return False
    return True

def verifyDfa1(tFunc):
...
if __name__ == "__main__":
    dfa = build_dfa1("inputs.txt")
    graph = dfa.minimize().trim().to_graphviz()
    graph = dfa.to_graphviz()
    graph.render("vis")
    # tFunc = {'S':{'1':'1'},'1':{'0':'3','1':'2'},'2':{'1':'D','0':'4'},'3':{'1':'1'},'4':{'1':'2'},'D':{}}
    tFunc = {'S': {'1': '1'}, '1': {'1': '2'}, '2': {'1': '3', '0': '1'}, '3': {'1': 'E', '0': '1'}, 'E': {}}
    tFunc = {'S': {'0': 'S', '1': '3'}, '3': {'1': '4', '0': 'E'}, '4': {'1': '5'}, 'E': {}, '5': {'1': '6'},
             '6': {'1': 'E'}}
    # print("verify: ",verifyDfa(tFunc))
    # cycleList, cycleDict = detect_cycles3(tFunc)
    # print("cycleList ",cycleList)
    # print("cycleDict ", cycleDict)
    # resDict = genSyms(cycleDict)
    # print("resDict ", resDict)
    # paths, nPaths = detect_cycles4(tFunc)
    # print("paths ",paths)
    # print("nPaths ", nPaths)
    # newPaths = genAllSyms(tFunc)
